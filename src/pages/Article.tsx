...
const Article = () => {
  ...
  const comments: (Comment & { children?: Comment[] })[] = useSelector(
  ...
  const getComments = () => {
    ...
    if (articleId) {
      res.push(
        ...comments.filter(({ article, parent }) => article === +articleId && parent === undefined),
      );
    }
  ...
  const renderComments = (comments: (Comment & { children?: Comment[] })[]) => {
    return comments?.map((comment: Comment & { children?: Comment[] }) => (
      <div
        ...
        style={{ marginBottom: '4px', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '4px' }}>
        {comment.text}
        <br />
        <form>
          {comment.id === commentAnswerId && (
            <>
              <textarea ref={refAnswer} name="reply_comment"></textarea>
              <br />
            </>
          )}
          <div style={{ cursor: 'pointer' }} onClick={() => handleOnAddAnswer(comment.id)}>
            <FontAwesomeIcon icon={faReply} style={{ marginLeft: '4px', color: 'blue' }} />
            <span style={{ marginLeft: '4px' }}>Reply</span>
          </div>
        </form>
        <br />
        {comment.children && (
          <div style={{ marginLeft: '8px' }}>{renderComments(comment.children)}</div>
        )}
      </div>
    ));
  };
    ...
    <main>
      ...
      <section className="block">
        ...
        <section style={{ marginTop: '32px' }}>
          <label>
            <textarea
              style={{ width: '98%', borderRadius: '4px' }}
              ref={refComment}
