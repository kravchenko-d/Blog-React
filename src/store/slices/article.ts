...
const articlesSlice = createSlice({
    ...
    reducers: {
        ...
        replyComment(state, action){ // не нужен для функционала с ответами на комментарии
            const {id, comment} = action.payload
            const index = state.articles.findIndex(article => article.id === id)
            if(~index){
                state.articles[index].comments.splice(state.articles[index].comments.indexOf(comment), 0, comment)
            }
        }
    }
})

export const {addArticles, deleteArticle, changeArticle, addComment, replyComment} = articlesSlice.actions
